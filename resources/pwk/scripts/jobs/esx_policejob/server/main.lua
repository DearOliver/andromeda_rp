if Config.EnableESXService then
	if Config.MaxInService ~= -1 then
		TriggerEvent('esx_service:activateService', 'lspd', Config.MaxInService)
	end
end

TriggerEvent('esx_phone:registerNumber', 'lspd', TranslateCap('alert_police'), true, true)
TriggerEvent('esx_society:registerSociety', 'lspd', TranslateCap('society_police'), 'society_police', 'society_police', 'society_police', {type = 'public'})
...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'lspd' then
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.job.name == 'lspd' then
			Wait(5000)
...
AddEventHandler('esx_policejob:forceBlip', function()
	for _, xPlayer in pairs(ESX.GetExtendedPlayers('job', 'lspd')) do
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_phone:removeNumber', 'lspd')
	end
end)
